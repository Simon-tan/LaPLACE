...
public class KKConditionalEstimator implements ConditionalEstimator {
    ...
    public Estimator getEstimator(double given) {
        ...
        double[] newWeight = new double[m_NumValues];
        ...
        for (int i = 0; i < m_NumValues; i++) {
            ...
            newWeight[i] = m_Weights[i] * (Statistics.normalProbability(zUpper) - Statistics.normalProbability(zLower));
            sumOfNewWeights += newWeight[i];
        }

        // We need this to rescale weights to get reasonable kernel width
        double factor = m_SumOfWeights / sumOfNewWeights;
        for (int i = 0; i < m_NumValues; i++) {
            result.addValue(m_Values[i], newWeight[i] * factor);
        }
        return result;
    }
    ...
    public String getRevision() {
        return RevisionUtils.extract("$Revision: 15522 $");
    }
